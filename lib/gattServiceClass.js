...
class gattService{
  // constructor(DBusClient = DBus.getBus('system'), rootNodeObj = DBus.registerService('system', 'com.netConfig').createObject('/com/netConfig'), sPath = '/com/netConfig',  UUID = '5a0379a8-d692-41d6-b51a-d1730ea6b9d6'){
    constructor(DBusClient, rootNodeObj = DBus.registerService('system', 'com.netConfig').createObject('/com/netConfig'), sPath = '/com/netConfig',  UUID = '5a0379a8-d692-41d6-b51a-d1730ea6b9d6'){
    this.DBusOld = {};
    this._DBusClient = DBusClient;
    // this._dBusService = DBusService;

    this._interfaceAddress =  'org.bluez.GattService1'  //.org.bluez.GattService1
    this.servicePath = sPath;
    this.UUID = UUID;
    this.prmary = true;

    // this._nodeObj = this._dBusService.createObject(this.servicePath);
    this._nodeObj = rootNodeObj;
    this._iface1 = this._nodeObj.createInterface(this._interfaceAddress);

    logit(this._iface1.DBus);
  ...
  createObjManagerIface(characteristics = []){   
    ...
    characteristics.forEach(function(char){
      ...
      objectDescription[_serviceNode+"/"+_node+"/userDescription"] = {
    ...
    //     '/com/sampleApp':{
    //       'org.bluez.GattService1':{
    //         'UUID': '003fb774-5a9e-4f0d-8bf5-77febd052666',
    //         'Primary': true
    //       }
    //     },
    //     '/com/sampleApp/cmd':{
    //       'org.bluez.GattCharacteristic1':{
    //         'UUID': '003fb774-5a9e-4f0d-8bf5-77febd052ee2',
    //         'Service': '/com/sampleApp',
    //         'Flags': ["encrypt-read", "encrypt-write", "notify"],
    //         'Notifying': false
    //       }
    //     },
    //     '/com/sampleApp/isAuthorized':{
    //       'org.bluez.GattCharacteristic1':{
    //         'UUID': '003fb774-5a9e-4f0d-8bf5-77febd052333',
    //         'Service': '/com/sampleApp',
    //         'Flags': ["read", "notify"],
    //         'Notifying': false
    //       }
 ...
 */
  registerGattService(){
    ...
    logit('please have a dbus object we can use')
    console.dir(this._nodeObj.service.bus.DBus, {depth:null})

    // var  bus = this._nodeObj.service.bus.DBus.getBus('system');  //worked but same error about introspeciton
    var  bus = this._nodeObj.service.bus
