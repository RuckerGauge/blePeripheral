...
class gattService{
  constructor(rootDBusObj = Dbus.registerService('system', 'com.netConfig').createObject('/com/netConfig'), sPath = "/com/netConfig",  UUID = "5a0379a8-d692-41d6-b51a-d1730ea6b9d6", ){
    ...
    // console.debug('gattServiceClass.js -> Exporting D-Bus interface for ' + this.node + ' Gatt service.');
    // this.DBusOld.exportInterface(this.ifProperties, this.node, this.ifaceDesc);
    this.iface1 = this._rootDBusObj.createInterface(this._interfaceAdd);

    this.iface1.addProperty('Primary', {
      type: {type: 'b'},
      getter: (callback)=>{
  ...
  createObjManagerIface(characteristics = []){   
    ...
    this.iface_ObjectManager = this._rootDBusObj.createInterface('org.freedesktop.DBus.ObjectManager');


    // this.iface_ObjectManager.addMethod('GetManagedObjects', {out: {type: 'a{oa{sa{sv}}}', name: 'dict_entry'}}, (cback)=>{
    this.iface_ObjectManager.addMethod('GetManagedObjects', {out: {type: 'a{oa{sa{sv}}}', name: 'dict_entry'}}, (cback)=>{
    ...
    this.iface_ObjectManager.update();
    ...
    // var iface = {
    //   GetManagedObjects: ()=> {
    //     console.debug('gattServiceClass.js -> GetManagedObjects method called. Responding with object description.');  
    //     console.debug('gattServiceClass.js -> __________________________________________________________________\n');
    //     console.dir(objectDescription, {depth: null});
    //     console.debug('gattServiceClass.js -> __________________________________________________________________');
    //     return objectDescription;
    //   }
    // };
    // this.DBusOld.exportInterface(iface, this.servicePath, ifaceDesc);
    // console.debug('gattServiceClass.js -> Added objectManager interface to Gatt Service.');
