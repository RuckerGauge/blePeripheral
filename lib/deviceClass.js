...
class device{
  ...
  setBooleanProperty(propertyName = 'Trusted', value = true, objectPath = '/org/bluez/hci0/dev_00_DB_70_C8_0C_7F'){
    ...
    let rsltObj = '';
    try{
      // rsltObj = cp.execSync('/usr/bin/gdbus call --system --dest org.bluez --object-path '+ objectPath +' --method org.freedesktop.DBus.Properties.Set org.bluez.Device1 ' + propertyName + ' <bool TRUE>' );
      // rsltObj = cp.execSync("/usr/bin/dbus-send --system --dest=com.rgMan --print-reply=literal /com/rgMan/gaugeAlert org.bluez.GattCharacteristic1.WriteValue string:" + "'" +  asArry + "'");
      rsltObj = cp.execSync('/usr/bin/dbus-send --system --dest=org.bluez --print-reply '+ objectPath +' org.freedesktop.DBus.Properties.Set string:"org.bluez.Device1" string:"' + propertyName + '" boolean:'+ value)
    } catch(err){
  ...
  setBooleanProperty_old(propertyName =  'Trusted', value = true, deviceObjPath = '/org/bluez/hci0/dev_00_DB_70_C8_0C_7F'){
