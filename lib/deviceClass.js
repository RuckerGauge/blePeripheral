...
class device{
  ...
  getProperty(propertyName =  'Trusted', objectPath = '/org/bluez/hci0/dev_00_DB_70_C8_0C_7F'){  
    logit('getting propertty '+ propertyName +' for ' + objectPath);
    let myBus = Dbus.getBus('system');
    myBus.getInterface(this.serviceName, objectPath, 'org.bluez.Device1', (err, iface)=>{
      if(err){
        console.error('Error getProperty dBus.getInterface('+ this.serviceName +', '+ objectPath +', org.bluez.Device1,).', err);
      } else {
        iface.getProperty(propertyName , function(err, value) {
          if(err){
            console.error('Error getting ' + propertyName, err);
          } else {
            logit(propertyName + ' = ' + value);
          };
        });
      };
    });
  ...
  getProperty_old(propertyName =  'Trusted', deviceObjPath = '/org/bluez/hci0/dev_00_DB_70_C8_0C_7F'){  
...
return promise;
  ...
  logAllProperties(objectPath = '/org/bluez/hci0/dev_00_DB_70_C8_0C_7F'){
    logit('logging all properties for ' + objectPath);
    this.dBus.getInterface(this.serviceName, objectPath, 'org.bluez.Device1', (err, iface)=>{
      if(err){
