...
class device{
  ...
  getProperty(propertyName =  'Trusted', objectPath = '/org/bluez/hci0/dev_00_DB_72_C8_0C_7F'){
    logit('objectPath = ' + objectPath);
    logit('propertyName = ' + propertyName);
    logit('interfaceAddress = ' + this._interfaceAddress);
        ...
        if(err){
          ...
          iface.Get(this._interfaceAddress, propertyName, (err, result)=>{
            if(err){
              ...
              if(result){
                  logit('getProperty called.  Result = ' + result);
                  reslove(result);
              } else {
                  logit('getProperty complete.');
                  reslove(result);
              };
            };
          });
    ...
    //   cp.exec('/usr/bin/gdbus call --system --dest org.bluez --object-path '+ objectPath+' --method org.freedesktop.DBus.Properties.Get ' + this._interfaceAddress + ' '+ propertyName, (error, stdOut, stdErr)=>{
    //     if (error){
  ...
  logAllProperties(objectPath = '/org/bluez/hci0/dev_00_DB_70_C8_0C_7F'){
    ...
    let rsltObj = {};
    try{
      let rsltBuffer = cp.execSync('/usr/bin/gdbus call --system --dest org.bluez --object-path '+ objectPath+' --method org.freedesktop.DBus.Properties.GetAll ' + this._interfaceAddress);
