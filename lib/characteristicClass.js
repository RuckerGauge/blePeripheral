...
class characteristic extends EventEmitter{
    constructor(dbus, sPath, UUID, node, flags, logIO = false){
        ...
        this.valueFormat = Buffer.from([0x25, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
        ...
        this.iface = {
            ReadValue: (options)=>{
                ...
                if(offset == 0){
                    ...
                    console.log('<<<<<<< ReadValue for ' + this.node +', from ' + device);
                    this.emit('ReadValue', device);
                } else {
                    console.log('<<<<<<< Sending next data chunk to ' + device);
                };
                var dataChunk = this.iface.Value.slice(offset, this.iface.Value.byteLength);
                //console.log('<<<<<<< [' + dataChunk.toString('hex') + ']');
                if(this.logIO){console.log('<<<<<<< [' + dataChunk.toString('utf8') + ']')};
                return dataChunk;
            },
            WriteValue:(arg1, options)=>{
                ...
                var device = optnObj.device;
                var offset = optnObj.offset;
                console.log('>>>>>>> writeValue for ' + this.node +', from ' + device);
                ...
                //console.log('>>>>>>> [' + this.iface.Value.toString('hex') + ']');
                if(this.logIO){console.log('>>>>>>> [' + this.iface.Value.toString('utf8') + ']')};
                return null;
            },
            StartNotify:()=>{
                console.log('Notify on for ' + this.node);
                this.iface.Notifying = true;
                return null;
            },
            StopNotify:()=>{
                console.log('Notify off for ' + this.node);
        ...
        console.log('Exporting D-Bus interface for ' + this.node + ' characteristic.');
        this.DBus.exportInterface(this.iface, this.iface.Service + '/' + this.node, this.ifaceDesc);
        this._descriptorUserDescription();
        this._descriptorCharacteristicFormat();
    };

    _descriptorUserDescription(){
        var descUUID = '2901-0000-1000-8000-00805f9b34fb'  //https://www.bluetooth.com/specifications/gatt/viewer?attributeXmlFile=org.bluetooth.descriptor.gatt.characteristic_user_description.xml
        ...
        this.userDescription_iface = {
            ...
            WriteValue:(arg1, options)=>{
                console.log('Caution unexpected behavor a write to the UserDescription was made and will be ignored...');
                return null;
            },
            UUID: descUUID,
        ...
        console.log('Exporting D-Bus interface for ' + this.userDescription_iface.Characteristic + ' Descriptor User Description.');
    ...
    _descriptorCharacteristicFormat(){
        var descUUID = '2904-0000-1000-8000-00805f9b34fb'  //https://www.bluetooth.com/specifications/gatt/viewer?attributeXmlFile=org.bluetooth.descriptor.gatt.characteristic_presentation_format.xml
        var descName = 'formatDescription'
        var charPath = this.servicePath + '/' + this.node;
        this.valueFormat_ifaceDesc = {
            name: 'org.bluez.GattDescriptor1',
            methods: {
                ReadValue:  ['', 'ay', [], ['arry{byte}']],
                WriteValue: ['ay', '', ['arry{byte}'], []]
        ...
        this.valueFormat_iface = {
            ReadValue: (options)=>{
                return this.valueFormat_iface.Value
            },
            WriteValue:(arg1, options)=>{
                console.log('Caution unexpected behavor a write to the UserDescription was made and will be ignored...');
                return null;
        ...
        console.log('Exporting D-Bus interface for ' + this.valueFormat_iface.Characteristic + ' Descriptor Presentation Format.');
        this.DBus.exportInterface(this.valueFormat_iface, this.iface.Service + '/' + this.node + '/' + descName, this.valueFormat_ifaceDesc);
    }
    ...
    setValue(valAsBuffer = this.iface.Value){
        if(Buffer.isBuffer(valAsBuffer)){
            ...
            console.log('Error not supported type. SetValue called with ' + valAsBuffer);
    ...
    notify(valAsBuffer = this.iface.Value){
        ...
        if(Buffer.isBuffer(valAsBuffer)){
            ...
            console.log('Error not supported type. notify called with ' + valAsBuffer);
            throw new Error('notify must be set with a buffer, string, or boolean. ');
        }
        if(this.iface.Notifying == false){
            console.log('Skipping this notification. No one has requested to be notified.')
            return
        };
        ...
        console.log('Sending notification for ' + this.node);
        this.DBus.sendSignal(path, iface, name, signature, notifyData);
        //console.log('<<<<<<< [' + buffToSend.toString('hex') + ']');
        if(this.logIO){console.log('<<<<<<< [' + buffToSend.toString('utf8') + ']')};
    };
