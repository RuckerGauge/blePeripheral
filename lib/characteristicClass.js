 ...
 * gdbus call --system --dest com.sampleApp --object-path /com/sampleApp --method org.bluez.GattCharacteristic1.ReadValue --timeout 5
 *  gdbus call [--system | --session | --address address] --dest bus_name --object-path /path/to/object --method org.project.InterfaceName.MethodName
...
class characteristic extends EventEmitter{
    constructor(DBusService = Dbus.registerService('system', 'com.netConfig') , sPath = '/com/netConfig', UUID, node, flags, logIO = false){
        ...
        this.iface1.addMethod('ReadValue',{out: {type: 'ay', name: 'arry{byte}'}}, (name, quality, cback)=>{
            ...
            // // return dataChunk;
            cback(null, 'okay cojt')
        });

        this.iface1.addMethod('WriteValue', {in: [{type: 'ay', name: 'arry{byte}'}] }, (arg1, options)=>{
            var optnObj = parseOptionsToObject(options)
            var device = optnObj.device;
            var offset = optnObj.offset;
