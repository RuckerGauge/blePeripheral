...
class characteristic extends EventEmitter{
    constructor(DBusService = Dbus.registerService('system', 'com.netConfig') , sPath = '/com/netConfig', UUID, node, flags, logIO = false){
        ...
        // this._iface2 = this.dBusService.createObject(this.servicePath + '/' + this.node + '/userDescription').createInterface('org.bluez.GattDescriptor1');
        ...
        this._iface2.addMethod('ReadValue',{in: [{type: '{sv}', name: 'options'}], out: {type: 'ay', name: 'value'} }, (options, cback)=>{
            var device = options.device;
            logit('Read description value called from ' + device);
            cback(null, [...this.servicePath]); //... is a spread operator used to expand Value 
        });
        ...
        this._iface2.addProperty('Value', {
            type: {type: 's'},
            getter: (callback)=>{
                callback(null, [...Buffer.from(this.node)]);
            }
        });
