 ...
 * dbus-send --system --dest=com.sampleApp --print-reply /com/sampleApp org.bluez.GattCharacteristic1.ReadValue array:string:[[device,x/y/z],[offset,0]]
 * dbus-send --system --dest=com.sampleApp --print-reply=literal /com/sampleApp org.bluez.GattCharacteristic1.WriteValue string:"tips" array:string:[[device,x/y/z],[offset,0]]
 * 
...
class characteristic extends EventEmitter{
    constructor(DBusService = Dbus.registerService('system', 'com.netConfig') , sPath = '/com/netConfig', UUID, node, flags, logIO = false){
        ...
        this.nodeObj = this.dBusService.createObject(this.servicePath + '/' + this.node);
        ...
        this.iface1.addMethod('ReadValue',{in: [{type: '{sv}', name: 'options'}], out: {type: 'ay', name: 'value'} }, (options, cback)=>{
            logit('ReadValue called');
        ...
        this.iface1.addMethod('WriteValue', {in: [{type: 'ay', name: 'value'}, {type: '{sv}', name: 'options'}] }, (arg1, options, cback)=>{
            logit('arg1 ->' + arg1 + '<-');
            logit('options ->' + options +'<-');
            logit('options follow:');
            ...
            if(this.listenerCount('WriteValue') == 0){
                ...
                this.ifProperties.Value = Buffer.from(arg1, 'utf-8')
            };        
