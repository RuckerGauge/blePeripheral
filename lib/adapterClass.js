...
class adapter{
    ...
    pairModeOn(booleanValue = false){
        console.debug('adapterClass.js -> setting pairable = ' + booleanValue);
    ...
    powerOn(booleanValue = false){
        console.debug('adapterClass.js -> setting power = ' + booleanValue);
    ...
    logAllProperties(deviceObjPath = '/org/bluez/hci0'){
        ...
        if(err){
          ...
          console.debug('adapterClass.js -> calling GetAll...');
          iface.GetAll(this[deviceName], (err,str) => {
            if (err) {
            console.error(`Error while calling GetAll: ${err}`);
            } else if (str) {
              console.debug('adapterClass.js -> Get All returned:');
              if(Array.isArray(str)){
                str.forEach(function(val, index){
                  var detailsObj = val[1][0][0]
                  console.debug('adapterClass.js -> \t' + val[0] + ' : ' + val[1][1] + ', of type -> ' + detailsObj.type + ' <-.');
                })
                return str
              } else {
                console.debug(str);
